...
| 已落地 MVP | 新增 AI PDF 编辑接口和短稿导出链路；前端生成 AI PDF 前先让 AI 整理，主界面不展示理由；页内空白真实落点留到截图门禁阶段。 |
...
## 2026-05-25 Agent 式 AI PDF 空白融入收口
| 阶段 | 状态 | 结果 |
|---|---|---|
| AI 编辑器 | complete | 用户勾选讲解卡后，后端二次调用模型做 include/drop、短补充压缩和 layout_intent 决策。 |
| 前端流程 | complete | “生成 AI PDF”先执行 AI 编辑，再导出；主界面不展示取舍理由。 |
| 短补充导出 | complete | PDF 只写 `pdf_snippet`，不把完整讲解卡塞入 PDF；被丢弃项只进入 manifest。 |
| 原页空白融入 | complete | `blank_note`、`margin_note`、`callout` 在有安全空白时直接写回复制后的源页。 |
| 拓展页 | complete | 页面放不下但仍重要的内容进入源页后的拓展页。 |
| 截图级门禁 | pending | 当前已做几何避让和人工截图检查，后续还需自动截图门禁判定文字裁剪和视觉效果。 |

## 2026-05-25 AI 笔记式融入路线纠偏
| 项 | 定法 |
|---|---|
| 路线判定 | “AI Explanation - Page N”追加解释页不符合最终愿景，只能算早期保守 fallback。 |
| 新目标 | `ai_guide.pdf` 应像学生在 PDF 上做笔记：在同一逻辑页内加入短旁注、箭头、下划线、高亮、公式说明和页边批注。 |
| 默认页数 | 默认保持 `ai_guide.pdf` 页数等于 `guide.pdf`；不再用新增解释页掩盖页内融入能力不足。 |
| 空间策略 | 原页空白优先；其次页边；再其次扩展同一页画布形成笔记边栏；仍放不下则缩短或报告无法安全放置。 |
| 内容策略 | Web 保留完整 AI 解释；PDF 只放 12-45 字的学习笔记型短句。 |
| 视觉策略 | 目标不是卡片报告，而是接近批注笔记：高亮词、蓝/红旁注、细箭头、局部公式解释、底部总结条。 |
| 实施计划 | `docs/superpowers/plans/2026-05-25-ai-notes-integration-route.md`。 |
| 下一阶段 | V5N：禁用默认追加解释页，建立 anchored note 数据结构、同页扩展画布和截图门禁。 |

## 2026-05-26 V5N 选块式 AI 笔记落地
| 项 | 定法 |
|---|---|
| 用户确认 | AI 笔记式融入路线可行，但 AI 内容来源继续按之前的选块模式。 |
| 入口规则 | `ai_guide.pdf` 默认只导出用户勾选且已有解释的块；整页解释保留在 Web 中作为阅读辅助，不自动进入 AI PDF。 |
| 导出规则 | 选块短笔记优先落在原页空白；放不下时扩展同一逻辑页的笔记边栏；禁止默认追加 `AI Explanation` 新页。 |
| 当前落地 | V5N-1 已完成主路线切换：后端不再追加解释页，前端导出只收集选中块，AI 编辑器不再建议 `extension_panel`，且不能通过 drop/漏项否决用户选择。 |
| 视觉门禁 | V5N-2 已补中文笔记渲染路径和截图冒烟，防止中文短补充在 `ai_guide.pdf` 中变问号或被长行裁切。 |
| 性能修复 | V5N-3 已修复 `ai_guide.pdf` 逐页复制共享资源导致的 GB 级膨胀；导出改为基于原 `guide.pdf` 加层另存。 |
| 样式优化 | V5N-4 已将 PDF 笔记升级为 `study_note_v2` 旁批样式，减少网页卡片感。 |

## 2026-05-27 转换与 AI 响应性能优化
| 阶段 | 状态 | 目标 |
|---|---|---|
| 1. 上下文恢复 | complete | 已读取 `lessons.md` 和现有计划，确认不能靠旧 job 验收，也不能牺牲选块式 AI PDF 主线。 |
| 2. 瓶颈定位 | complete | 确认转换链路重复 PDF rasterize、`pages=[]` 误触发全页检查；AI 链路存在前端单并发队列和重复整理请求。 |
| 3. 回归测试 | complete | 新增 `test_v6_performance_flow.py`，覆盖空页检查、预览图复用、AI 队列并发、整页本地缓存和 AI PDF 编辑复用。 |
| 4. 实现优化 | complete | 视觉检查复用 `guide_preview` 图并保持 1.2 倍检查语义；前端块解释改为 2 并发，重复整页/编辑请求走本地缓存。 |
| 5. 验证自查 | complete | 全量单测、后端编译、前端语法、diff 检查通过；已重启 8765，真实转换 `test.pptx` 并检查 `guide.pdf` 渲染截图。 |
